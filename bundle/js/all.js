...
;
(function () {
	var slick = $('.PortfolioCarusel__slides').slick({
		slidesToShow: 3,
		infinite: true,
		arrows: true,
		// autoplay: true,
		prevArrow: $('.PortfolioCarusel__arrowPrev'),
		nextArrow: $('.PortfolioCarusel__arrowNext'),
		responsive: [{
			breakpoint: 992,
			settings: {
				slidesToShow: 2
	...
	$('[data-PortfolioCarusel-filter="all"]').addClass('PortfolioCarusel__filter_active');
	$('[data-PortfolioCarusel-filter]').on('click', function (e) {
		var item = $(e.target).parents(".PortfolioCarusel__filter").data('PortfolioCarusel-filter');
		if (item === undefined) {
			item = $(e.target).data('PortfolioCarusel-filter');
		} else {
			item;
		}
		if (item === 'all') {
			$('[data-PortfolioCarusel-filter]').removeClass('PortfolioCarusel__filter_active');
			$('[data-PortfolioCarusel-filter="' + item + '"]').addClass('PortfolioCarusel__filter_active');
			$('.PortfolioCarusel__slides').slick('slickUnfilter');
		} else {
			$('[data-PortfolioCarusel-filter]').removeClass('PortfolioCarusel__filter_active');
			$('.PortfolioCarusel__slides').slick('slickUnfilter');
			$('[data-PortfolioCarusel-filter="' + item + '"]').addClass('PortfolioCarusel__filter_active');
			var filterItem = $('.PortfolioCarusel__slides').slick('slickFilter', '.' + item);
			filterItem.find('.PortfolioCarusel__item').addClass('animated zoomIn');
		}
	});
})();
